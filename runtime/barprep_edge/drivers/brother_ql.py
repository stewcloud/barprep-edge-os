...
class BrotherQLDriver(PrinterDriver):
    def discover(self) -> list[OutputDevice]:
        try:
            ...
            return [OutputDevice(
                id=settings.printer_uri, name=f"Brother {settings.printer_model}",
                driver="brother_ql", manufacturer="Brother", model=settings.printer_model,
                connection="usb", connection_uri=settings.printer_uri, ready=False,
                media_label=settings.printer_label,
                media_description=settings.printer_label_description, detail=str(exc))]
        return [OutputDevice(
            id=uri, name=f"Brother {settings.printer_model}", driver="brother_ql",
            manufacturer="Brother", model=settings.printer_model, connection="usb",
            connection_uri=uri, ready=True, media_label=settings.printer_label,
            media_description=settings.printer_label_description) for uri in uris]
    ...
    def print_png(self, png_data: bytes, *, copies: int = 1, cut: bool = True,
        ...
        instructions = convert(
            qlr=raster, images=[image] * copies, label=settings.printer_label,
            rotate="auto", threshold=70.0, dither=False, compress=True,
            red=False, dpi_600=False, hq=True, cut=cut)
