...
def create_diagnostics_zip(printers: list[dict[str, object]]) -> bytes:
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        archive.writestr("manifest.json", json.dumps({
            ...
            "type": "barprep-edge-diagnostics"}, indent=2))
        ...
        archive.writestr("service-log.txt",
                         _command("journalctl", "-u", "barprep-edge", "-n", "250", "--no-pager"))
        archive.writestr("wifi-watchdog-log.txt",
                         _command("journalctl", "-u", "barprep-edge-wifi", "-n", "250", "--no-pager"))
